...
The following configuration `plugins.toml` example is the quickest way for you 
to use CLI gateway to start with one plugin-managed observability exporter and no extra layering:
...
### Successful Shortest Path Output Indications

The following are indications of a successful output:
...
Only the open-ended parts of your code config (including specified components and
